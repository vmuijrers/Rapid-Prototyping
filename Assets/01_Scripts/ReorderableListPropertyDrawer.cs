...
public class ReorderableListPropertyDrawer : PropertyDrawer
{
    ...
    private void Initialize(SerializedProperty property)
    {
        ...
        genericType = GetListType(property.managedReferenceValue.GetType(), fieldName);
    }

    static Type GetListType(System.Type type, string fieldName)
    {
    ...
    private void OnAddDropdown(Rect buttonRect, ReorderableList list)
    {
        ...
        // Convert the list of types to a list of class names
        List<string> classNames = new List<string>();
        foreach (Type type in derivedTypes)
        {
            classNames.Add(type.FullName);
        }
        foreach (string typeName in classNames)
        {
            ObjectHandle handle = Activator.CreateInstance(Assembly.GetAssembly(genericType).FullName, typeName);
            var item = handle.Unwrap();
            menu.AddItem(new GUIContent(typeName), false, OnAddButtonClick, item);
        }
